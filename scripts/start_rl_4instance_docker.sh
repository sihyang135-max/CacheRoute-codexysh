...
docker exec -e PYTHONDONTWRITEBYTECODE=1 \
  -e KDN_TEXT_DB_DIR="${KDN_TEXT_DB_DIR:-$PROJECT_IN_CONTAINER/log/rl4/runtime/kdn-text-db}" \
  -e KDN_KV_DB_DIR="${KDN_KV_DB_DIR:-$PROJECT_IN_CONTAINER/log/rl4/runtime/kdn-kv-db}" \
  -e MODEL_DIR="$MODEL_DIR" -e MODEL_NAME="$MODEL_NAME" \
